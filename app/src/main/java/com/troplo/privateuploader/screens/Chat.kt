    ...
    bottomBar = {
      ...
      Row(
              ...
              Row(
                ...
                typersState.value.let { typers ->
                  if (typers.isNotEmpty()) {
                    val names = typers.filter { it.chatId == ChatStore.getChat()?.id }.map { it.user.username }
                    Text(
        ...
        Box(
          ...
          IconButton(
            onClick = {
              chatViewModel.sendMessage(token, associationId, message.value, context, editId.value)
              message.value = ""
              editId.value = 0
            }
          ) {
      ...
      LazyColumn(
        ...
        messages.value.value?.forEach { msg ->
          item(
            ...
            Message(msg, compact(msg, messages.value.value!!), messageCtx, messageCtxMessage)
          }
        }
      }
    }
...
class ChatViewModel : ViewModel() {
  ...
  init {
    ...
    socket?.on("message") {
      ...
      val existingMessage = messages.value?.find { e -> e.id == message.id || (e.content == message.content && e.pending == true && e.userId == message.userId)}
      ...
      if(existingMessage == null) {
        ...
        val newMessage = message.copy(pending = false, error = false, content = message.content, createdAt = message.createdAt)
  ...
  fun sendMessage(token: String, associationId: Int, message: String, context: Context, editId: Int = 0) {
    viewModelScope.launch(Dispatchers.IO) {
