    ...
    AlertDialog(
        ...
        confirmButton = {
            LoadingButton(
                ...
                onClick = {
                    viewModel.renameCollection(name.value, collectionId, open)
                },
...
class RenameCollectionViewModel: ViewModel() {
    val loading = mutableStateOf(false)

    fun renameCollection(name: String, collectionId: Int, open: MutableState<Boolean>) {
        loading.value = true
        viewModelScope.launch(Dispatchers.IO) {
            val response = TpuApi.retrofitService.updateCollection(collectionId, UpdateCollectionRequest(
                name = name
            )).execute()
