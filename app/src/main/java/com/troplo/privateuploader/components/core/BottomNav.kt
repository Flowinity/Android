...
fun BottomBarNav(
    ...
    closePanels: () -> Unit,
    isAMOLED: State<Boolean> = mutableStateOf(false)
) {
    ...
    AnimatedVisibility(
        ...
        exit = shrinkVertically(animationSpec = spring(stiffness = Spring.StiffnessHigh)) {
            ...
            NavigationBar(
                tonalElevation = if(isAMOLED.value) 0.dp else NavigationBarDefaults.Elevation,
...
fun BottomBarNavPreview() {
    ...
    BottomBarNav(navController = navController, panelState = panelState, closePanels = {})
}
